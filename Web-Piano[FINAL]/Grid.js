    ...
    for (var i=0;i<this.notePos.length;i++){
      for (var j=0;j<this.notePos[i].length;j++){
        ...
        if (this.noteState[i][j]>0) {
          ...
          var c1 = color(155,0,155,alpha);
          var c2 = color(0,205,155,alpha);
            
            
            //new code - We create colour sliders to change the colour of the ellipses
            var gridColour_change = color('hsl(' + colourSlider_change.value() + ' , 100%, 50% )');
            
            var mix = lerpColor(gridColour_change, c2, map(i, 0, this.notePos.length, 0, 1));
        ...
        this.noteState[i][j]=constrain(this.noteState[i][j],0,1);
          ...
          //new code - used for random position
